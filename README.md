...
In the `backend/coupons` directory, add a file called `coupons.csv` with a content like below:
```csv
COUPON,MULTIPLIER
myCoupon1,0.10
myCoupon2,0.70
```
Using the coupon myCoupon1 applies a 90% discount, reducing the price to 10% of the original. For instance, if the original price is $10, the discounted price becomes $1.
Using the coupon myCoupon2 applies a 30% discount, reducing the price to 70% of the original. For example, a $10 item would now cost $7.
...
- **Configure the instance**
  ...
  - **Install Gunicorn and Configure Systemd Service**
    ...
    - Create Gunicorn service: `sudo nano /etc/systemd/system/gunicorn.service`
      ...
      ExecStart=/home/ubuntu/questions_solver/backend/venv/bin/gunicorn --workers 1 --bind 127.0.0.1:8000 --timeout 600 app:app
